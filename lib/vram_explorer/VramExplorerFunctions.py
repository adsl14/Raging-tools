...
from lib.vram_explorer.VramExplorerVars import VramExplorerVars
...
def open_spr_file(spr_path, start_pointer):
    ...
    with open(spr_path, mode='rb') as file:
        ...
        VramExplorerVars.stpk_struct.data_offset = int.from_bytes(file.read(VramExplorerVars.bytes2Read), "big")

        # Check if we're dealing with a RB2 to RB1 port file
        file.seek(VramExplorerVars.stpk_struct.data_offset)
        if file.read(VramExplorerVars.bytes2Read).hex() != VramExplorerVars.STPK:
            VramExplorerVars.single_stpk_header = True
        else:
            file.seek(VramExplorerVars.stpk_struct.data_offset + start_pointer)
            VramExplorerVars.stpk_struct.data_offset = int.from_bytes(file.read(VramExplorerVars.bytes2Read), "big") + 64
            VramExplorerVars.single_stpk_header = False

        # Create the VramExplorerVars.sprp_struct instance
        file.seek(VramExplorerVars.stpk_struct.data_offset + 20)
        VramExplorerVars.sprp_struct.type_info_base = VramExplorerVars.stpk_struct.data_offset + 64
        VramExplorerVars.sprp_struct.string_base = VramExplorerVars.sprp_struct.type_info_base + int.from_bytes(file.read(VramExplorerVars.bytes2Read), "big")
        file.seek(VramExplorerVars.stpk_struct.data_offset + 24)
        VramExplorerVars.sprp_struct.data_info_base = VramExplorerVars.sprp_struct.string_base + int.from_bytes(file.read(VramExplorerVars.bytes2Read), "big")
        file.seek(VramExplorerVars.stpk_struct.data_offset + 28)
        VramExplorerVars.sprp_struct.data_base = VramExplorerVars.sprp_struct.data_info_base + int.from_bytes(file.read(VramExplorerVars.bytes2Read), "big")
        file.seek(VramExplorerVars.sprp_struct.type_info_base + 8)
        VramExplorerVars.sprp_struct.data_count = int.from_bytes(file.read(VramExplorerVars.bytes2Read), "big")
        ...
        if data_type != bytes.fromhex('00'):
            ...
            file.seek(VramExplorerVars.sprp_struct.string_base + 1)
            texture_name = ""
            counter = 0
            record_byte = True
            while True:
                ...
                if record_byte:
                    if data == bytes.fromhex('2E'):
                        ...
                        for tx2_data_element in VramExplorerVars.tx2_datas:
                            while True:
                        ...
                        VramExplorerVars.tx2_datas.append(tx2_data)
                        texture_name = ""
                        counter += 1
                        if counter == VramExplorerVars.sprp_struct.data_count:
                            break
                        record_byte = False
                        continue
        ...
        VramExplorerVars.offset_quanty_difference = np.zeros(VramExplorerVars.sprp_struct.data_count)

        # Get the data info (TX2D)
        file.seek(VramExplorerVars.sprp_struct.data_info_base)
        for i in range(0, VramExplorerVars.sprp_struct.data_count):
            sprp_data_info = SprpDataInfo()

            # Move where the information starts
            file.seek(8, os.SEEK_CUR)
            sprp_data_info.name_offset = int.from_bytes(file.read(VramExplorerVars.bytes2Read), "big")
            sprp_data_info.data_offset = int.from_bytes(file.read(VramExplorerVars.bytes2Read), "big")
            sprp_data_info.dataSize = int.from_bytes(file.read(VramExplorerVars.bytes2Read), "big")
            VramExplorerVars.sprpDatasInfo.append(sprp_data_info)

            # Move to the next start offset
            file.seek(12, os.SEEK_CUR)

        # Get the data itself
        for sprpDataInfo in VramExplorerVars.sprpDatasInfo:
            tx2_d_info = Tx2dInfo()

            # Move where the information starts
            file.seek(VramExplorerVars.sprp_struct.data_base + sprpDataInfo.data_offset)

            # Move where the information starts
            file.seek(4, os.SEEK_CUR)
            tx2_d_info.data_offset = int.from_bytes(file.read(VramExplorerVars.bytes2Read), "big")
            tx2_d_info.data_offset_old = tx2_d_info.data_offset
            file.seek(4, os.SEEK_CUR)
            tx2_d_info.data_size = int.from_bytes(file.read(VramExplorerVars.bytes2Read), "big")
            ...
            VramExplorerVars.tx2d_infos.append(tx2_d_info)
    ...
    with open(vram_path, mode="rb") as file:
        ...
        if VramExplorerVars.stpz_file:
            ...
            if VramExplorerVars.single_stpk_header:
                ...
                file.seek(16)
                texture_offset = int.from_bytes(file.read(VramExplorerVars.bytes2Read), "big")
            ...
            # VramExplorerVars.STPK file RB2 to RB1 port (has double VramExplorerVars.STPK file type)
                ...
                file.seek(16 + 64)
                texture_offset = int.from_bytes(file.read(VramExplorerVars.bytes2Read), "big") + 64

            # The size of the file is in position 20
            VramExplorerVars.vram_file_size_old = int.from_bytes(file.read(VramExplorerVars.bytes2Read), "big")
            ...
            VramExplorerVars.vram_file_size_old = VramExplorerVars.tx2d_infos[0].data_size
        ...
        for i in range(0, len(VramExplorerVars.tx2d_infos)):

            # Creating DXT5 and DXT1 heading
            if VramExplorerVars.tx2d_infos[i].dxt_encoding != 0:

                header_2 = VramExplorerVars.tx2d_infos[i].height.to_bytes(4, 'little') + VramExplorerVars.tx2d_infos[i].width.to_bytes(4, 'little') + (
                    VramExplorerVars.tx2d_infos[i].data_size).to_bytes(4, 'little')

                header_3_2 = VramExplorerVars.tx2d_infos[i].mip_maps.to_bytes(4, 'little')

                header_3 = bytes.fromhex(header_3_1) + header_3_2 + bytes.fromhex(header_3_3)

                header_4, header_5, header_6 = create_header(VramExplorerVars.tx2d_infos[i].dxt_encoding)
                header = header_1 + header_2 + header_3 + header_4 + header_5 + header_6

                # Store the data in memory
                file.seek(VramExplorerVars.tx2d_infos[i].data_offset + texture_offset)
                data = file.read(VramExplorerVars.tx2d_infos[i].data_size)
                VramExplorerVars.tx2_datas[i].data = header + data

            # Creating RGBA heading
            else:

                header_1_bmp = "42 4D"
                header_2_bmp = (VramExplorerVars.tx2d_infos[i].data_size + 54).to_bytes(4, 'little').hex()
                header_3_bmp = "00 00 00 00 36 00 00 00 28 00 00 00"
                header_4_1_bmp = VramExplorerVars.tx2d_infos[i].width.to_bytes(4, 'little').hex()
                header_4_2_bmp = VramExplorerVars.tx2d_infos[i].height.to_bytes(4, 'little').hex()
                ...
                if VramExplorerVars.tx2d_infos[i].height == 1:
                ...
                if VramExplorerVars.tx2_datas[i].extension == "png":
                    ...
                    with open("tempSwizzledImage", mode="wb") as file_temp:
                        file_temp.write(VramExplorerVars.tx2_datas[i].data)

                    # Run the exe file of 'swizzle.exe' with the option '-u' to unswizzle the image
                    args = os.path.join(VramExplorerVars.swizzle_path) + " \"" + "tempSwizzledImage" + "\" \"" + "-u" + "\""
                    os.system('cmd /c ' + args)

                    # Get the data from the .exe
                    with open("tempUnSwizzledImage", mode="rb") as file_temp:
                        VramExplorerVars.tx2_datas[i].data_unswizzle = file_temp.read()
                    with open("Indexes.txt", mode="r") as file_temp:
                        VramExplorerVars.tx2_datas[i].indexes_unswizzle_algorithm = file_temp.read().split(";")[:-1]
                    ...
                    VramExplorerVars.tx2_datas[i].data_unswizzle = header + VramExplorerVars.tx2_datas[i].data_unswizzle
...
def action_item(q_model_index, image_texture, encoding_image_text, mip_maps_image_text, size_image_text):
    ...
    if VramExplorerVars.current_selected_texture != q_model_index.row():
        VramExplorerVars.current_selected_texture = q_model_index.row()
        ...
        if VramExplorerVars.tx2d_infos[VramExplorerVars.current_selected_texture].dxt_encoding != 0:
            # Create the dds in disk and open it
            show_dds_image(image_texture, VramExplorerVars.tx2_datas[VramExplorerVars.current_selected_texture].data,
                           VramExplorerVars.tx2d_infos[VramExplorerVars.current_selected_texture].width, VramExplorerVars.tx2d_infos[VramExplorerVars.current_selected_texture].height)
        else:
            if VramExplorerVars.tx2_datas[VramExplorerVars.current_selected_texture].extension != "png":
                show_bmp_image(image_texture, VramExplorerVars.tx2_datas[VramExplorerVars.current_selected_texture].data,
                               VramExplorerVars.tx2d_infos[VramExplorerVars.current_selected_texture].width, VramExplorerVars.tx2d_infos[VramExplorerVars.current_selected_texture].height)
            else:
                show_bmp_image(image_texture, VramExplorerVars.tx2_datas[VramExplorerVars.current_selected_texture].data_unswizzle,
                               VramExplorerVars.tx2d_infos[VramExplorerVars.current_selected_texture].width, VramExplorerVars.tx2d_infos[VramExplorerVars.current_selected_texture].height)
        ...
        mip_maps_image_text.setText("Mipmaps: %s" % VramExplorerVars.tx2d_infos[VramExplorerVars.current_selected_texture].mip_maps)
